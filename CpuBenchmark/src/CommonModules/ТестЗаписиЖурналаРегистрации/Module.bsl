...
Процедура ВыполнитьЗаписьВЖурналРегистрации(Знач КоличествоИтераций) Экспорт
	
	
	Для Сч = 0 По КоличествоИтераций Цикл
			
		ЗаписьЖурналаРегистрации("ТестЗаписиЖурналаРегистрации.ВыполнитьЗаписьВЖурналРегистрации",
		УровеньЖурналаРегистрации.Ошибка, , , "Тест записи" , );
...
Процедура ЗапуститьТестМногопоточно(Знач КоличествоИтераций, Знач КоличествоПотоков, ДанныеЗамера) Экспорт
	...
	Для Сч=1 По КоличествоПотоков Цикл
		ФоновыеЗадания.Выполнить("ТестЗаписиЖурналаРегистрации.ВыполнитьЗаписьВЖурналРегистрации",ПараметрыФЗ,ИмяФЗ+"_"+XMLСтрока(Сч),ИмяФЗ);		
	КонецЦикла;			
...
Функция ИмяТеста() Экспорт
	Возврат "Тест записи журнала регистрации";
КонецФункции
...
Функция ПолучитьИмяФЗ()
	Возврат "ТестЗаписиЖурналаРегистрации";
КонецФункции
